...
设置里提供 10 组预设配色，以及**两个取色器**——任意两种颜色都能混搭（主色决定按钮、标识、光晕；辅色决定渐变另一端与背景第二光斑）。

**颜色不能直接套用，必须反推。** 这是这一块的核心难点：

- 亮青 `#12c4c9` 上配白字只有 **2.9:1**，读不清。所以按钮上的字色要按对比度**自动选深或浅**。
- 任选一个中间调（比如 `#808080`）时，**两种字色都到不了 4.5:1**。所以还必须能**自动挪底色**：
  - 按钮底色朝字色的反方向推进，直到达标（`ensureContrast`）
  - 用户气泡底色从主色压暗，直到白字达标（`readableOn`）

实测 17 种取色（含 10 个预设 + 中灰 / 纯黄 / 纯白 / 纯黑 / 暗红 / 荧光绿 / 深灰等极端值），按钮字与气泡字**全部 ≥ 4.5:1**；纯黑、暗红、深灰会自动把字色翻成白色。

**特效：**

- **切换扩散动画**：从点击位置用 `clip-path: circle()` 扩开一整屏。用圆形裁剪做扩散比动渐变位置便宜得多（渐变本身在 CSS 里不可过渡）。
- **背景光晕流动**：三个光斑各自以 29 / 34 / 41 秒漂移。因为玻璃后面一直在变，透过玻璃看到的颜色也在极缓慢地变化。
- **呼吸辉光**（品牌标记）与**思考点脉动**。

**性能与无障碍：**

- 光晕流动与扩散动画**都可以关掉**：玻璃后方持续变化会让 iOS 不停重算模糊层、增加耗电；关掉后玻璃质感仍在。
- 系统开启 `prefers-reduced-motion` 时，两者自动失效。注意选择器必须写成 `*, *::before, *::after` —— **`*` 只匹配元素、匹配不到伪元素**，否则呼吸辉光、脉动、流式光标在"减弱动态效果"下仍会继续动。
- 品牌图形（Logo / 头像）的 `fill` 由 CSS 覆盖并加了过渡，所以换色时是渐变而不是跳变。SVG 的 `fill` 是表现属性，CSS 优先级更高。
- 主题存在 `localStorage`，刷新后保留。
