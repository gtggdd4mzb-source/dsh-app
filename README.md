...
深色主题随应用图标配色（深海军蓝 + 青绿），设计 token 集中在 `styles.css` 顶部的 `:root`：层次（页面→顶栏→卡片→浮层逐级提亮）、描边、文字三级、语义色、圆角与阴影。

几个刻意的取舍：

- **强调色只用在可点击处**。亮青（`#12c4c9`）配深色字对比度约 7.6:1，用于发送键等按钮；用户气泡改用深青渐变承载正文，因为亮青上放白字只有约 2.9:1，读不清。
- **代码块带语言标签与复制按钮**，复制走事件委托，流式输出中途已渲染的代码块也能立刻复制。
- **支持表格、引用、分隔线**：模型输出表格很常见，窄屏下已收紧字号与内边距让 4 列放得下。
- **顶栏滚动后加阴影**，强化"内容在下方滚动"的层次。
